...
use uuid::Uuid;
use witness_core::{
    Result, ingestion::IngestionService, signing::SigningKeypair, storage::Storage, types::*,
};
...
enum Commands {
    ...
    Csv {
        ...
        value_col: String,
        #[arg(long)]
        unit_col: Option<String>,
        #[arg(long)]
        lat_col: Option<String>,
        #[arg(long)]
        lon_col: Option<String>,
        #[arg(long)]
        station_col: Option<String>,
        #[arg(long)]
        instrument_col: String,
        #[arg(long)]
...
async fn main() -> Result<()> {
    ...
    let keypair = if let Some(path) = cli.key_file {
        ...
        let decoded = if bytes.len() == 32 {
            ...
            hex::decode(String::from_utf8_lossy(&bytes).trim()).map_err(|_| {
                witness_core::WitnessError::Crypto(
                    "Key must be 32 raw bytes or a 64-character hex string".to_string(),
                )
            })?
        };
    ...
    match cli.command {
        Commands::Observe {
            ...
            let measurement = Measurement {
                ...
                value: MeasuredValue {
                    ...
                    text: if value.parse::<f64>().is_err() {
                        Some(value)
                    } else {
                        None
                    },
        ...
        Commands::Csv {
            ...
            let mapping = witness_core::ingestion::CsvColumnMapping {
                quantity: quantity_col,
                value_numeric: value_col.clone(),
                value_text: value_col,
                unit: unit_col.unwrap_or_default(),
                latitude: lat_col.unwrap_or_default(),
                longitude: lon_col.unwrap_or_default(),
                station_id: station_col.unwrap_or_default(),
                instrument_id: instrument_col,
                timestamp: timestamp_col,
                ..Default::default()
            };

            let nodes = service
                .ingest_csv_observations(&file, author, domain, mapping, keypair.as_ref())
                .await?;
